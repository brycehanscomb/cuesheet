...
export function cuesheet(): CueSheet {
	...
	function processRepeatingCue(
		...
		const interval = c.interval as number;
		...
		if (!state) {
			state = { lastFireTime: c.startTime - interval, count: 0 };
			repeatState.set(c, state);
		}

		let nextFire = state.lastFireTime + interval;
